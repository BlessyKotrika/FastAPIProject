  ...
  const fetchDashboard = async () => {
    try {
      ...
      const res = await todayService.getActions({
        user_id: profile.user_id || 'test_user',
        crop: profile.crop || 'wheat',
        location: profile.location || 'Barabanki',
        sowing_date: profile.sowing_date || '2025-11-01',
  ...
  return (
    <Layout>
      <div className="p-4 space-y-6">
        ...
        >
          <div className="absolute -right-4 -top-4 w-32 h-32 bg-primary-500/10 rounded-full blur-3xl"></div>
          <div className="absolute -left-4 -bottom-4 w-24 h-24 bg-accent-500/10 rounded-full blur-2xl"></div>
          ...
          >
            ...
            {loading ? (
              <div className="space-y-2">
                ...
                <div className="h-2 bg-slate-50 rounded w-2/3 animate-pulse"></div>
              </div>
            ) : (
